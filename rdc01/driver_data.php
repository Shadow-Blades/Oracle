<?php
	...
	if($_REQUEST['id'] != null){
     ...
     $id=$_REQUEST['id']; 
        ...
        $sql=sqlsrv_query($connection,"SELECT TOP(5)  POD.PO_NUMBER as 'ponumber',role.id as roleid, role.role_name as rolename,  cu_orders.VEHICLE_NUMBER as 'vehiclenumber',driver.id as 'driverid',vendor.user_email as 'vendoremail',
		...
		LEFT JOIN cu_userroles as role on role.id=driver.role_id
         ...
         where driver.id=$id order by cu_orders.id desc", array(), array( "Scrollable" => 'static' ));
        ...
        if($num >0)
        {
			...
			while($data= sqlsrv_fetch_array($sql,SQLSRV_FETCH_ASSOC)){
                ...
                $lastaction = date_format($datevalneedby, 'd/m/Y H:i:s');
                ...
                if($data['complete'] == '1' ){
                    ...
                    "grossweight"=>(int)$data['grossweight'],"tareweight"=>(int)$data['tareweight'],"actiontime"=>$lastaction,"drivernamee"=>$data['drivername'],
                    "itemname"=>$data['itemname'],"netweight"=>(int)$data['netweight'],"vendormobile"=>$data['vendormobile'],"drivermobile"=>$data['drivermobile']
                    ,"challanno"=>$data['challanno'],"sitename"=>$data['sitename'],"vehiclenumber"=>$data['vehiclenumber']
                    ,"creationtime"=>$date_cration ,"vendoremail"=>$data['vendoremail'],"rdcnetweight"=>(int)$data['rdcnet']));
                    $completecounter++;
                ...
                elseif($data['placed'] == '1' && $data['active'] == '0' &&  $data['cancel'] == '0'){
                    ...
                    "grossweight"=>(int)$data['grossweight'],"tareweight"=>(int)$data['tareweight'],"vendormobile"=>$data['vendormobile'],
                    "itemname"=>$data['itemname'],"netweight"=>(int)$data['netweight'],"actiontime"=>$lastaction,"drivermobile"=>$data['drivermobile'],"drivernamee"=>$data['drivername']
                    ,"challanno"=>$data['challanno'],"sitename"=>$data['sitename'],"vehiclenumber"=>$data['vehiclenumber']
                    ,"creationtime"=>$date_cration ,"vendoremail"=>$data['vendoremail']));
                    $pendingcounter++;
                }
                elseif($data['active'] == '0' && $data['cancel']== '1'){
                    ...
                    "itemname"=>$data['itemname'],"actiontime"=>$lastaction,"vendormobile"=>$data['vendormobile'],"drivermobile"=>$data['drivermobile'],"drivernamee"=>$data['drivername']
                    ,"challanno"=>$data['challanno'],"sitename"=>$data['sitename'],"vehiclenumber"=>$data['vehiclenumber']
                    ,"creationtime"=>$date_cration ,"vendoremail"=>$data['vendoremail']));
                    $cancelcounter++;
                }
