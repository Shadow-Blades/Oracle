    <?php
        ...
        $poid=$_REQUEST['id']; 
            ...
            cu_orders.TARE as 'tareweight',cu_orders.GROSS as 'grossweight',cu_orders.id as 'orderid',Table_1.invoicenumber,Table_1.invoicedate,Table_1.status,Table_1.erpdescription as 'remarks',Table_1.name,Table_1.date,Table_1.approved_by,
            POD.ITEM_NAME as 'itemname',Table_1.qty,Table_1.amount,Table_1.sapstatus,Table_1.bookedamount,
            ...
            where POD.PO_NUMBER='$poid' and ostatus.complate !=0   and (cu_orders.MRN_NO is not null and cu_orders.MRN_NO !='0' and cu_orders.MRN_NO!='' ) and cu_orders.RDC_NET !='0'
            and cu_orders.created_at  > '2025-01-01 00:00:00.000'  AND ostatus.actiontime IS NOT NULL order by cu_orders.created_at desc", array(), array( "Scrollable" => 'static' ));
            ...
            if($num >0)
            {
                ...
                while($data= sqlsrv_fetch_array($sql, SQLSRV_FETCH_ASSOC)){
                    ...
                    if($data['qty'] == null || $data['sapstatus'] == 'CANCELLED' ){
                        ...
                        "action_by"=>$data['approved_by'],"status"=>$data['status'],"challan_no1"=>$data['challan_no1'],"NET1"=>$data['NET1'],"remarks"=>$data['remarks'],"invoicenumber"=>$data['invoicenumber'],"invoicedate"=>$data['invoicedate']));
                        $pendingcounter++;
                    
                    }
                      if(  $data['approved_by'] == 2 ){

                    array_push($avaitingforapproval,array("orderid"=>$data['orderid'],"ponumber"=>$data['ponumber'],"vendorname"=>$data['vendorname'],
                        "grossweight"=>(int)$data['grossweight'],"tareweight"=>(int)$data['tareweight'],"actiontime"=>$date_creationtime,
                        "rdcgross"=>(int)$data['rdcgross'],"rdctare"=>(int)$data['rdctare'],"rdcnet"=>(int)$data['rdcnet'],
                        "itemname"=>$data['itemname'],"netweight"=>(int)$data['netweight'],"mrnno"=>$data['mrnno']
                        ,"challanno"=>$data['challanno'],"sitename"=>$data['sitename'],"vehiclenumber"=>$data['vehiclenumber']
                        ...
                        ,"moisture"=>$data['moisture'],"reciept"=>$data['reciept'],"uom"=>$data['uom'],"acceptvy"=>$data['acceptby'],
                        "drivermobile"=>$data['drivermobile'],"invoicename"=>$data['name'],"invoiceremarks"=>$data['remarks'],"invoicedate"=>$data['date'],
                        "action_by"=>$data['approved_by'],"status"=>$data['status'],"challan_no1"=>$data['challan_no1'],"NET1"=>$data['NET1'],"remarks"=>$data['remarks'],
                        "invoicenumber"=>$data['invoicenumber'],"invoicedate"=>$data['invoicedate'],"amount"=>$data['amount'],"bookedamount"=>$data['bookedamount'],"sapstatus"=>$data['sapstatus']));
                        $activecounter++;
                    }
